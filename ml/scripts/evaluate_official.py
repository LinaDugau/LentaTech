...
def evaluate(video: str, pred_dir: Path | None = None):
    gt_path = DATA / video / f"{video}.csv"
    root = pred_dir or OUT
    pred_path = root / f"final_eval_{video}.csv"
    if not pred_path.exists():
        pred_path = root / f"final_{video}.csv"
...
def main():
    ...
    ap.add_argument("--pred-dir", type=Path, default=None,
                    help="директория с final_eval_*.csv (по умолчанию ml/output)")
    args = ap.parse_args()
    pred_root = args.pred_dir or OUT
    overall_n_gt = 0
    overall_success = 0
    for v in args.videos:
        ...
        if not gt_path.exists():
            continue
        gt = pd.read_csv(gt_path).rename(columns=GT_COLUMN_RENAME)
        pred_path = pred_root / f"final_eval_{v}.csv"
        if not pred_path.exists():
            pred_path = pred_root / f"final_{v}.csv"
        if not pred_path.exists():
