...
def _norm_barcode_digits(x) -> str:
    s = str(x or "").strip()
    if s.lower() in ("", "nan"):
        return ""
    if s.endswith(".0"):
...
def tag_confidence_score(r: dict) -> int:
    """Чем выше — тем надёжнее строка для финального CSV."""
    score = 0
    bc = _norm_barcode_digits(r.get("barcode"))
    if len(bc) == 13 and _ean_checksum_ok(bc):
        score += 50
    elif len(bc) >= 12:
        score += 18
    if _has_price_field(r.get("price_default")) or _has_price_field(r.get("price_card")):
        score += 30
    qr = _norm_barcode_digits(r.get("qr_code_barcode"))
    if len(qr) >= 12:
        score += 22
    if str(r.get("id_sku") or "").strip():
        score += 8
    if str(r.get("print_datetime") or "").strip():
        score += 5
...
def _row_rank(r: dict) -> tuple:
    bc = _norm_barcode_digits(r.get("barcode"))
    area = (int(r["x_max"]) - int(r["x_min"])) * (int(r["y_max"]) - int(r["y_min"]))
    return (1 if len(bc) >= 12 else 0, tag_confidence_score(r), area)
...
def parse_barcode(text: str, raw_bc: str) -> str:
    """EAN из декодера или одного OCR-токена.

    13 цифр принимаем только с валидной checksum; 12 — как частичный код
    (декодер / GT). Не склеиваем цифры из разных токенов.
    """
    candidates: list[str] = []
    ...
    for c in candidates:
        if len(c) == 13 and _ean_checksum_ok(c):
            return c
        if len(c) == 12:
            return c
    return ""
...
CODE_RX_FULL = re.compile(r"(?<!\d)(0\d{5})\s*[-–—]?\s*(0\d{5})(?!\d)")
CODE_RX_UNDERSCORE_DASH = re.compile(r"(?<!\d)(\d{1,2})_(0\d{5})\s*[-–—]\s*(0\d{5})(?!\d)")
CODE_RX_UNDERSCORE = re.compile(r"(?<!\d)(\d{1,2})_(0\d{5})(?!\d)")
_SKU_PREFIXES = ("270", "370", "470", "170", "570", "770")


def is_valid_layout_code(code: str) -> bool:
    """Код зоны выкладки, не фрагмент id_sku (270xxx)."""
    if not code or str(code).strip().lower() in ("", "нет", "nan"):
        return False
    s = str(code).strip()
    head = re.sub(r"\D", "", s)[:6]
    if any(head.startswith(p) for p in _SKU_PREFIXES):
        return False
    if re.match(r"\d{1,2}_0\d{5}", s.replace(" ", "")):
        return True
    return bool(re.match(r"0\d{5}\s*[-–—]\s*0\d{5}", s))
...
def parse_code(text: str) -> str:
    ...
    if m:
        ...
        return out if is_valid_layout_code(out) else "нет"
...
def main():
    ...
    ap.add_argument("--min_confidence", type=int, default=15,
                    help="отбросить строки с confidence ниже порога (0=выкл)")
    ap.add_argument("--dedup_ms", type=int, default=3000,
                    help="окно дедупа по bbox+timestamp, мс (default 3s)")
    args = ap.parse_args()
    ...
    if not args.no_filter:
        rows = [r for r in rows if is_real_tag(r)]
    print(f"Filtered: {n_before} -> {len(rows)} rows (price / barcode / QR only)")

    if not args.no_dedup:
        rows_sorted = sorted(rows, key=lambda r: _row_rank(r), reverse=True)
        kept: list[dict] = []
        for r in rows_sorted:
            ...
            if not merged:
                kept.append(r)
        print(f"Dedup ({args.dedup_ms}ms IoU≥0.5): {len(rows)} -> {len(kept)} rows")
        rows = kept

    if args.min_confidence > 0:
        n_pre = len(rows)
        rows = [r for r in rows if tag_confidence_score(r) >= args.min_confidence]
        print(f"Confidence≥{args.min_confidence}: {n_pre} -> {len(rows)} rows")
