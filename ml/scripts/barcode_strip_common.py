...
def build_digit_groups(n_digits: int = N_DIGITS, n_groups: int = 8) -> list[tuple[int, int]]:
    """7–10 перекрывающихся сегментов EAN для чтения с разных кадров."""
    n_groups = max(2, min(n_groups, n_digits + 2))
    if n_groups <= 2:
        return [(0, n_digits)]
    width = max(2, (n_digits + 2) // (n_groups - 1))
    step = max(1, (n_digits - width + max(0, n_groups - 2)) // max(1, n_groups - 2))
    spans: list[tuple[int, int]] = []
    start = 0
    while start < n_digits and len(spans) < n_groups - 1:
        end = min(n_digits, start + width)
        if end - start >= 1:
            spans.append((start, end))
        start += step
    if not spans or spans[-1][1] < n_digits:
        tail = (max(0, n_digits - width), n_digits)
        if tail not in spans:
            spans.append(tail)
    spans.append((0, n_digits))
...
def extract_group_strip(
    ...
    *,
    ...
    """Crop digit positions [start:end) from normalized strip."""
    if strip is None or strip.size == 0:
        return None
    h, w = strip.shape[:2]
    slot_w = w / N_DIGITS
    x0 = max(0, int((start - pad) * slot_w))
    x1 = min(w, int((end + pad) * slot_w))
    if x1 - x0 < 6:
        return None
    patch = strip[:, x0:x1]
    if upscale > 1:
        patch = cv2.resize(
            patch,
            (max(8, patch.shape[1] * upscale), max(STRIP_H, patch.shape[0] * upscale)),
            interpolation=cv2.INTER_CUBIC,
        )
    return patch


def slots_to_group_digits(slots: np.ndarray, start: int, end: int) -> np.ndarray:
    """Subset of slot patches for group [start:end)."""
